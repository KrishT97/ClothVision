...
class ModelTrainer:
    ...
    def __init__(self, model, optimizer, loss_fn, epochs, early_stopper, device, lr_scheduler):
        self.device = device
        self.model = model
    ...
    def train(self, training_loader):
        ...
        for i, data in enumerate(training_loader):
            ...
            acc_sum += (outputs.argmax(1) == labels.argmax(1)).type(torch.float).sum().item() / (height * width)
    ...
    def validate(self, val_loader):
        ...
        with torch.no_grad():
            for inputs, labels in val_loader:
                ...
                val_loss += self.loss_fn(outputs, labels.argmax(1)).item()
                batch_size, _, height, width = outputs.shape
                correct += (outputs.argmax(1) == labels.argmax(1)).type(torch.float).sum().item() / (height * width)
    ...
    def test(self, test_loader):
        ...
        with torch.no_grad():
            for inputs, labels in test_loader:
                inputs, labels = inputs.to(self.device), labels.to(self.device)
                outputs = self.model(inputs)
                test_loss += self.loss_fn(outputs, labels.argmax(1)).item()
                batch_size, _, height, width = outputs.shape
                correct += (outputs.argmax(1) == labels.argmax(1)).type(torch.float).sum().item() / (height * width)
